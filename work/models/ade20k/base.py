"""Modified from https://github.com/CSAILVision/semantic-segmentation-pypaddle"""
...
class NormalizeTensor:
    ...
    def __call__(self, tensor):
        ...
        dtype = tensor.dtype
        mean = paddle.as_tensor(self.mean, dtype=dtype, device=tensor.device)
        std = paddle.as_tensor(self.std, dtype=dtype, device=tensor.device)
...
class ModelBuilder:
    ...
    def weights_init(m):
        ...
        if classname.find('Conv') != -1:
            ...
            m.bias_attr.data.fill_(1e-4)

    @staticmethod
    def build_encoder(arch='resnet50dilated', fc_axis=512, weights=''):
        ...
        if len(weights) > 0:
            ...
            net_encoder.load_state_dict(
                paddle.load(weights, map_location=lambda storage, loc: storage), strict=False)
        return net_encoder

    @staticmethod
    def build_decoder(arch='ppm_deepsup',
                      fc_axis=512, num_class=NUM_CLASS,
    ...
    @staticmethod
    def get_decoder(weights_path, arch_encoder, arch_decoder, fc_axis, drop_last_conv, *arts, **kwargs):
        path = os.path.join(weights_path, 'ade20k', f'ade20k-{arch_encoder}-{arch_decoder}/decoder_epoch_20.pth')
        return ModelBuilder.build_decoder(arch=arch_decoder, fc_axis=fc_axis, weights=path, use_softmax=True, drop_last_conv=drop_last_conv)

    @staticmethod
    def get_encoder(weights_path, arch_encoder, arch_decoder, fc_axis, segmentation,
                    *arts, **kwargs):
        if segmentation:
            path = os.path.join(weights_path, 'ade20k', f'ade20k-{arch_encoder}-{arch_decoder}/encoder_epoch_20.pth')
        else:
            path = ''
        return ModelBuilder.build_encoder(arch=arch_encoder, fc_axis=fc_axis, weights=path)


def conv3x3_bn_relu(in_planes, out_planes, stride=1):
    return nn.Sequential(
        nn.Conv2D(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, bias_attr=False),
        BatchNorm2D(out_planes),
        nn.ReLU(),
    )


class SegmentationModule(nn.Layer):
    def __init__(self,
                 weights_path,
                 num_classes=150,
    ...
    def multi_mask_from_multiclass(self, pred, classes):
        ...
        return isin(pred, paddle.to_tensor(classes,dtype='int64'))
    ...
    def predict(self, tensor, imgSizes=(-1,),  # (300, 375, 450, 525, 600)
        ...
        with paddle.no_grad():
            ...
            scores = paddle.zeros([1, NUM_CLASS, segSize[0], segSize[1]])
            features = paddle.zeros([1, self.feature_maps_channels, segSize[0], segSize[1]])
            ...
            _, pred = paddle.max(scores, axis=1)
    ...
    def get_edges(self, t):
        edge = paddle.cuda.ByteTensor(t.size()).zero_()
        ...
        if True:
            return edge.half()
        return edge.cast("float32")


# pyramid pooling, deep supervision
class PPMDeepsup(nn.Layer):
    def __init__(self, num_class=NUM_CLASS, fc_axis=4096,
        ...
        for scale in pool_scales:
            self.ppm.append(nn.Sequential(
                ...
                nn.Conv2D(fc_axis, 512, kernel_size=1, bias_attr=False),
                BatchNorm2D(512),
                nn.ReLU()
            ))
        self.ppm = nn.LayerList(self.ppm)
        self.cbr_deepsup = conv3x3_bn_relu(fc_axis // 2, fc_axis // 4, 1)

        self.conv_last = nn.Sequential(
            nn.Conv2D(fc_axis + len(pool_scales) * 512, 512,
        ...
        self.conv_last_deepsup = nn.Conv2D(fc_axis // 4, num_class, 1, 1, 0)
    ...
    def forward(self, conv_out, segSize=None):
        ...
        ppm_out = paddle.concat(ppm_out, 1)
        ...
        if self.drop_last_conv:
            ...
            _ = nn.functional.log_softmax(_, axis=1)
...
class MobileNetV2Dilated(nn.Layer):
...
class C1DeepSup(nn.Layer):
    def __init__(self, num_class=150, fc_axis=2048, use_softmax=False, drop_last_conv=False):
        super(C1DeepSup, self).__init__()
        self.use_softmax = use_softmax
        self.drop_last_conv = drop_last_conv

        self.cbr = conv3x3_bn_relu(fc_axis, fc_axis // 4, 1)
        self.cbr_deepsup = conv3x3_bn_relu(fc_axis // 2, fc_axis // 4, 1)

        # last conv
        self.conv_last = nn.Conv2D(fc_axis // 4, num_class, 1, 1, 0)
        self.conv_last_deepsup = nn.Conv2D(fc_axis // 4, num_class, 1, 1, 0)
...
# last conv
class C1(nn.Layer):
    def __init__(self, num_class=150, fc_axis=2048, use_softmax=False):
...
# pyramid pooling
class PPM(nn.Layer):
    def __init__(self, num_class=150, fc_axis=4096,
                 use_softmax=False, pool_scales=(1, 2, 3, 6)):
