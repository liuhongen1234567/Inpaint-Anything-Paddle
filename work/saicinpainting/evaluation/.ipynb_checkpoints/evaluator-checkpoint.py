...
class InpaintingEvaluator():
    def __init__(self, dataset, scores, area_grouping=True, bins=10, batch_size=32, device='cuda',
        ...
        """
        :param dataset: paddle.utils.data.Dataset which contains images and masks
        ...
        self.device = paddle.device(device)
    ...
    def _get_bin_edges(self):
        ...
        for batch in self.dataloader:
            ...
            area = mask.to(self.device).reshape(batch_size, -1).mean(axis=-1)
            ...
            with paddle.no_grad():
                ...
                for batch in tqdm.auto.tqdm(self.dataloader, desc=score_name, leave=False):
                    ...
                    if self.clamp_image_range is not None:
                        image_batch = paddle.clamp(image_batch,
...
class InpaintingEvaluatorOnline(nn.Layer):
    def __init__(self, scores, bins=10, image_key='image', inpainted_key='inpainted',
        ...
        self.scores = nn.LayerDict(scores)
    ...
    def _get_bins(self, mask_batch):
        ...
        area = mask_batch.view(batch_size, -1).mean(axis=-1).detach().cpu().numpy()
    ...
    def process_batch(self, batch: Dict[str, paddle.Tensor]):
        return self(batch)
