...
from paddle.optim import Adam, SGD 
...
def _pyrdown(im : paddle.Tensor, downsize : tuple=None):
    """downscale the image"""
    if downsize is None:
...
def _pyrdown_mask(mask : paddle.Tensor, downsize : tuple=None, eps : float=1e-8, blur_mask : bool=True, round_up : bool=True):
...
def _erode_mask(mask : paddle.Tensor, ekernel : paddle.Tensor=None, eps : float=1e-8):
...
def _l1_loss(
    pred : paddle.Tensor, pred_downscaled : paddle.Tensor, ref : paddle.Tensor, 
    mask : paddle.Tensor, mask_downscaled : paddle.Tensor, 
    image : paddle.Tensor, on_pred : bool=True
    ):
    """l1 loss on src pixels, and downscaled predictions if on_pred=True"""
    loss = paddle.mean(paddle.abs(pred[mask<1e-8] - image[mask<1e-8]))
    if on_pred: 
        loss += paddle.mean(paddle.abs(pred_downscaled[mask_downscaled>=1e-8] - ref[mask_downscaled>=1e-8]))                
    return loss

def _infer(
    image : paddle.Tensor, mask : paddle.Tensor, 
    forward_front : nn.Layer, forward_rears : nn.Layer, 
    ref_lower_res : paddle.Tensor, orig_shape : tuple, devices : list, 
    ...
    masked_image = paddle.concat([masked_image, mask], axis =1)
    ...
    ekernel = paddle.from_numpy(cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(15,15)).astype(bool)).float()
...
def refine_predict(
    batch : dict, inpainter : nn.Layer, gpu_ids : str, 
    ...
    devices = [paddle.device(gpu_id) for gpu_id in gpu_ids]
