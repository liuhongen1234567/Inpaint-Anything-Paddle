...
class PairwiseScore(EvaluatorScore, ABC):
    ...
    def get_value(self, groups=None, states=None):
        ...
        """
        individual_values = paddle.concat(states, axis=-1).reshape(-1).cpu().numpy() if states is not None \
...
class FIDScore(EvaluatorScore):
    ...
    def get_value(self, groups=None, states=None):
        ...
        activations_pred = paddle.concat(activations_pred).cpu().numpy()
        activations_target = paddle.concat(activations_target).cpu().numpy()
