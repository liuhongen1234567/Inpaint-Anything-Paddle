...
class PerceptualLoss(paddle.nn.Layer):
    ...
    def forward(self, pred, target, normalize=True):
        ...
        Output pypaddle Variable N long
        """
...
def normalize_tensor(in_feat, eps=1e-10):
    norm_factor = paddle.sqrt(paddle.sum(in_feat ** 2, axis =1, keepaxis=True))
...
def np2tensor(np_obj):
    ...
    return paddle.Tensor(np_obj[:, :, :, np.newaxis].transpose((3, 2, 0, 1)))
...
def im2tensor(image, imtype=np.uint8, cent=1., factor=255. / 2.):
    return paddle.Tensor((image / factor - cent)
...
class BaseModel(paddle.nn.Layer):
    ...
    def load_network(self, network, network_label, epoch_label):
        ...
        network.load_state_dict(paddle.load(save_path, map_location='cpu'))
...
class DistModel(BaseModel):
    ...
    def initialize(self, model='net-lin', net='alex', colorspace='Lab', pnet_rand=False, pnet_tune=False,
        ...
        if (self.model == 'net-lin'):  # pretrained net + linear layer
            ...
            if (not is_train):
                self.net.load_state_dict(paddle.load(model_path, **kw), strict=False)
        ...
        if self.is_train:  # training mode
            ...
            self.lr = lr
            self.old_lr = lr
            self.optimizer_net = paddle.optim.Adam(self.trainable_parameters, lr=lr, betas=(beta1, 0.999))
        else:  # test mode
            self.net.eval()

        # if (use_gpu):
            # self.net.to(gpu_ids[0])
            # self.net = paddle.nn.DataParallel(self.net, device_ids=gpu_ids)
            # if (self.is_train):
    ...
    def forward(self, in0, in1, retPerLayer=False):
        ...
        INPUTS
            in0, in1 - paddle.Tensor object of shape Nx3xXxY - image patch scaled to [-1,1]
        OUTPUT
    ...
    def clamp_weights(self):
        for module in self.net.modules():
            if (hasattr(module, 'weight') and module.kernel_size == (1, 1)):
                module.weight.data = paddle.clamp(module.weight.data, min=0)
    ...
    def forward_train(self):  # run forward pass
        ...
        # print(paddle.norm(self.net.module.net.slice1[0].weight).item(), paddle.norm(self.net.module.lin0.model[1].weight).item())
...
def score_2afc_dataset(data_loader, func, name=''):
    ...
    INPUTS
        ...
        func - callable distance function - calling d=func(in0,in1) should take 2
            pypaddle tensors with shape Nx3xXxY, and return numpy array of length N
    OUTPUTS
...
def score_jnd_dataset(data_loader, func, name=''):
    ...
    INPUTS
        ...
        func - callable distance function - calling d=func(in0,in1) should take 2
            pypaddle tensors with shape Nx3xXxY, and return pypaddle array of length N
    OUTPUTS
...
def spatial_average(in_tens, keepaxis=True):
    return in_tens.mean([2, 3], keepaxis=keepdim)
    ...
    def forward(self, in0, in1, retPerLayer=False):
        ...
        if (self.lpips):
            if (self.spatial):
                ...
                res = [spatial_average(self.lins[kk].model(diffs[kk]), keepaxis=True) for kk in range(self.L)]
        else:
            if (self.spatial):
                res = [upsample(diffs[kk].sum(axis =1, keepaxis=True), out_H=in0.shape[2]) for kk in range(self.L)]
            else:
                res = [spatial_average(diffs[kk].sum(axis =1, keepaxis=True), keepaxis=True) for kk in range(self.L)]
...
class ScalingLayer(nn.Layer):
    def __init__(self):
        ...
        self.register_buffer('shift', paddle.Tensor([-.030, -.088, -.188])[None, :, None, None])
        self.register_buffer('scale', paddle.Tensor([.458, .448, .450])[None, :, None, None])
...
class Dist2LogitLayer(nn.Layer):
    ...
    def __init__(self, chn_mid=32, use_sigmoid=True):
        ...
        layers = [nn.Conv2D(5, chn_mid, 1, stride=1, padding=0, bias_attr=True), ]
        layers += [nn.LeakyReLU(0.2, True), ]
        layers += [nn.Conv2D(chn_mid, chn_mid, 1, stride=1, padding=0, bias_attr=True), ]
        layers += [nn.LeakyReLU(0.2, True), ]
        layers += [nn.Conv2D(chn_mid, 1, 1, stride=1, padding=0, bias_attr=True), ]
        if (use_sigmoid):
            layers += [nn.Sigmoid(), ]
        self.model = nn.Sequential(*layers)

    def forward(self, d0, d1, eps=0.1):
        return self.model(paddle.concat((d0, d1, d0 - d1, d0 / (d1 + eps), d1 / (d0 + eps)), axis =1))
...
class L2(FakeNet):
    ...
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if (self.colorspace == 'RGB'):
            (N, C, X, Y) = in0.size()
            value = paddle.mean(paddle.mean(paddle.mean((in0 - in1) ** 2, axis =1).view(N, 1, X, Y), axis=2).view(N, 1, 1, Y),
                               axis=3).view(N)
            return value
            ...
            ret_var = Variable(paddle.Tensor((value,)))
            # if (self.use_gpu):
...
import paddle
from paddlevision import models as tv
