...
class BaseAdversarialLoss:
    def pre_generator_step(self, real_batch: paddle.Tensor, fake_batch: paddle.Tensor,
                           generator: nn.Layer, discriminator: nn.Layer):
        """
    ...
    def pre_discriminator_step(self, real_batch: paddle.Tensor, fake_batch: paddle.Tensor,
                               generator: nn.Layer, discriminator: nn.Layer):
        """
    ...
    def generator_loss(self, real_batch: paddle.Tensor, fake_batch: paddle.Tensor,
                       discr_real_pred: paddle.Tensor, discr_fake_pred: paddle.Tensor,
    ...
    def discriminator_loss(self, real_batch: paddle.Tensor, fake_batch: paddle.Tensor,
                           discr_real_pred: paddle.Tensor, discr_fake_pred: paddle.Tensor,
                           mask: Optional[paddle.Tensor] = None) \
            -> Tuple[paddle.Tensor, Dict[str, paddle.Tensor]]:
        """
...
def make_r1_gp(discr_real_pred, real_batch):
    if paddle.is_grad_enabled():
        grad_real = paddle.autograd.grad(outputs=discr_real_pred.sum(), inputs=real_batch, create_graph=True)[0]
        grad_penalty = (grad_real.view(grad_real.shape[0], -1).norm(2, axis =1) ** 2).mean()
    else:
        grad_penalty = 0
...
class BCELoss(BaseAdversarialLoss):
    ...
    def generator_loss(self, discr_fake_pred: paddle.Tensor) -> Tuple[paddle.Tensor, Dict[str, paddle.Tensor]]:
        real_mask_gt = paddle.zeros(discr_fake_pred.shape).to(discr_fake_pred.device)
    ...
    def discriminator_loss(self,
                           mask: paddle.Tensor,
                           discr_real_pred: paddle.Tensor,
                           discr_fake_pred: paddle.Tensor) -> Tuple[paddle.Tensor, Dict[str, paddle.Tensor]]:

        real_mask_gt = paddle.zeros(discr_real_pred.shape).to(discr_real_pred.device)
