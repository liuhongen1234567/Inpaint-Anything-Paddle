...
class MultidilatedConv(nn.Layer):
    def __init__(self, in_dim, out_dim, kernel_size, dilation_num=3, comb_mode='sum', equal_axis=True,
        ...
        for i in range(dilation_num):
            ...
            if i > 0 and shared_weights:
                ...
                convs[-1].bias_attr = convs[0].bias_attr
            dilation *= 2
        ...
        if self.shuffle_in_channels:
            ...
            self.register_buffer('in_channels_permute', paddle.tensor(in_channels_permute))
    ...
    def forward(self, x):
        ...
        if self.cat_in:
            if self.equal_dim:
                x = x.chunk(len(self.convs), axis =1)
            else:
                new_x = []
                start = 0
        ...
        if self.cat_out:
            out = paddle.concat(outs, axis =1)[:, self.index]
        else:
            out = sum(outs)
        return out
