...
class ResnetBlock(nn.Layer):
    def __init__(self, dim, padding_type, norm_layer, activation=nn.ReLU(True), use_dropout=False, conv_kind='default',
                 dilation=1, in_axis=None, groups=1, second_dilation=None):
        ...
        self.conv_block = self.build_conv_block(dim, padding_type, norm_layer, activation, use_dropout,
                                                conv_kind=conv_kind, dilation=dilation, in_axis=in_dim, groups=groups,
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, activation, use_dropout, conv_kind='default',
                         dilation=1, in_axis=None, groups=1, second_dilation=1):
...
class MultidilatedResnetBlock(nn.Layer):
...
class MultiDilatedGlobalGenerator(nn.Layer):
...
class ConfigGlobalGenerator(nn.Layer):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3,
                 n_blocks=3, norm_layer=nn.BatchNorm2D,
                 padding_type='reflect', conv_kind='default',
                 deconv_kind='convtranspose', activation=nn.ReLU(True),
...
class GlobalGenerator(nn.Layer):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2D,
        ...
        dilated_block_kwargs = dict(axis=feats_num_bottleneck, padding_type=padding_type,
...
class GlobalGeneratorFromSuperChannels(nn.Layer):
    def __init__(self, input_nc, output_nc, n_downsampling, n_blocks, super_channels, norm_layer="bn", padding_type='reflect', add_out_act=True):
        ...
        if type(norm_layer) == functools.partial:
            use_bias_attr = (norm_layer.func == nn.InstanceNorm2D)
        else:
            use_bias_attr = (norm_layer == nn.InstanceNorm2D)

        channels = self.convert_super_channels(super_channels)
        self.channels = channels

        model = [nn.ReflectionPad2d(3),
                 nn.Conv2D(input_nc, channels[0], kernel_size=7, padding=0, bias_attr=use_bias_attr),
                 norm_layer(channels[0]),
                 nn.ReLU(True)]

        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model += [nn.Conv2D(channels[0+i], channels[1+i], kernel_size=3, stride=2, padding=1, bias_attr=use_bias_attr),
        ...
        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i)
            model += [nn.Conv2DTranspose(channels[n_downsampling+3+i],
                                           channels[n_downsampling+3+i+1],
                                           kernel_size=3, stride=2,
                                           padding=1, output_padding=1,
                                           bias_attr=use_bias_attr),
