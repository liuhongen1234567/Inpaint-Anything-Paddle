...
class ReflectPad2d(nn.Layer):
    def __init__(self, padding, pad_type):
        super().__init__()
        self.padding = _ntuple(4)(padding)
        self.pad_type = pad_type
    def forward(self, x):
        return F.pad(x, self.padding,self.pad_type)

class ResNetHead(nn.Layer):
    def __init__(self, input_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2D,
        ...
        model = [ReflectPad2d(3,pad_type='reflect'),
...
class ResNetTail(nn.Layer):
    def __init__(self, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2D,
        ...
        for i in range(n_downsampling):
            ...
            model += [nn.Conv2DTranspose(ngf * mult, int(ngf * mult / 2), kernel_size=3, stride=2, padding=1,
                                         output_padding=1),
        ...
        for _ in range(out_extra_layers_n):
            out_layers += [nn.Conv2D(ngf, ngf, kernel_size=1, padding=0),
                           up_norm_layer(ngf),
                           up_activation]
        out_layers += [ReflectPad2d(3,pad_type='reflect'),
...
class MultiscaleResNet(nn.Layer):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=2, n_blocks_head=2, n_blocks_tail=6, n_scales=3,
        ...
        self.heads = nn.LayerList([ResNetHead(input_nc, ngf=ngf, n_downsampling=n_downsampling,
        ...
        self.tails = nn.LayerList([ResNetTail(output_nc,
    ...
    def forward(self, ms_inputs: List[paddle.Tensor], smallest_scales_num: Optional[int] = None) \
        ...
        for i in range(len(ms_features)):
            ...
            if prev_tail_features is not None:
                ...
                cur_tail_input = paddle.concat((cur_tail_input, prev_tail_features), axis =1)
...
class MultiscaleDiscriminatorSimple(nn.Layer):
    def __init__(self, ms_impl):
        super().__init__()
        self.ms_impl = nn.LayerList(ms_impl)
    ...
    def forward(self, ms_inputs: List[paddle.Tensor], smallest_scales_num: Optional[int] = None) \
            -> List[Tuple[paddle.Tensor, List[paddle.Tensor]]]:
        """
        :param ms_inputs: List of inputs of different resolutions from HR to LR
        :param smallest_scales_num: int or None, number of smallest scales to take at input
...
class DiscriminatorMultiToSingleOutputStackedMixin:
    ...
    def forward(self, x):
        ...
        out = paddle.concat(scaled_outs, axis =1)
